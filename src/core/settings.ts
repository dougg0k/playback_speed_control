...
function sanitizeShortcuts(value: Partial<ShortcutConfig> | undefined): ShortcutConfig {
  return {
...
export function sanitizeSettings(value: Partial<AppSettings> | undefined): AppSettings {
  return {
    ...
    preferredSpeed: clampSpeed(value?.preferredSpeed ?? DEFAULT_SETTINGS.preferredSpeed),
    speedStep: Math.min(4, Math.max(0.05, Number((value?.speedStep ?? DEFAULT_SETTINGS.speedStep).toFixed(2)))),
    rememberLastSpeed: value?.rememberLastSpeed ?? DEFAULT_SETTINGS.rememberLastSpeed,
    ...
    disabledSites: normalizeRules(value?.disabledSites ?? DEFAULT_SETTINGS.disabledSites),
...
function sanitizePlaybackState(
  ...
  const siteLastSpeedEntries = Object.entries(value?.siteLastSpeed ?? {}).filter(
    ([siteKey, speed]) => Boolean(siteKey) && typeof speed === "number",
  );
  ...
  return {
    ...
    siteLastSpeed: Object.fromEntries(
      siteLastSpeedEntries.map(([siteKey, speed]) => [siteKey, clampSpeed(speed)]),
    ),
  };
}
...
export async function getSettings(): Promise<AppSettings> {
  ...
  return sanitizeSettings(stored[SETTINGS_KEY] as Partial<AppSettings> | undefined);
}
...
export function listenForSettingsChanges(
  ...
  const handler = (
    ...
    callback(sanitizeSettings(changes[SETTINGS_KEY].newValue as Partial<AppSettings>));
  };
