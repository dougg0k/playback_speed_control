...
export class MediaRegistry {
  ...
  private getEligibleMedia(): HTMLMediaElement[] {
    ...
    return Array.from(this.media).filter((element) => {
      ...
      if (!settings.workOnAudio && element instanceof HTMLAudioElement) return false;
  ...
  private registerNode(node: Node): boolean {
    ...
    const visit = (candidate: Node): void => {
      if (candidate instanceof HTMLVideoElement || candidate instanceof HTMLAudioElement) {
        changed = this.registerMediaElement(candidate) || changed;
      }

      if (!(candidate instanceof Element || candidate instanceof ShadowRoot)) return;

      const elements = "querySelectorAll" in candidate ? candidate.querySelectorAll("*") : [];
