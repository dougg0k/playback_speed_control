...
function App() {
  ...
  const sendAction = async (message: ContentRequestMessage) => {
    ...
    try {
      ...
      setPopupStatus((current) => ({
        ...current,
        state: null,
        unavailableReason: "This page is not currently exposing controllable media.",
      }));
    } finally {
  ...
  const currentHostname = popupStatus.hostname || popupStatus.state?.hostname || "";
  ...
  if (!isReady) {
    return <div className="popup-shell"><div className="loading-state">Loading…</div></div>;
  }
    ...
    <div className="popup-shell">
      ...
      <section className="hero-card">
        ...
        <div className="hero-meta">
          <span className={`status-pill ${popupStatus.state?.hasMedia ? "is-active" : ""}`}>
            {popupStatus.state?.hasMedia ? popupStatus.state?.activeKind ?? "media" : "No media"}
          </span>
          {currentHostname ? <span className="hostname">{currentHostname}</span> : null}
        </div>
      </section>
        ...
        <div className="inline-field-row">
          <label className="field field-inline">
            ...
            <input
              ...
              onChange={(event) =>
                void saveSettings({
                  preferredSpeed: parseSpeedInput(event.target.value, settings.preferredSpeed),
                })
              }
            />
          </label>
          ...
          >
            {currentSiteDisabled ? "Enable on this site" : "Disable on this site"}
          </button>
        </div>
        ...
        <section className="settings-panel">
          <div className="settings-group">
            <div className="section-heading">
              <h3>Playback</h3>
              <span>Keep the frequent adjustments compact and predictable.</span>
            </div>
            <div className="grid-two">
              <label className="field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    void saveSettings({
                      speedStep: parseSpeedInput(event.target.value, settings.speedStep),
                    })
                  }
                />
              </label>
          ...
          <div className="settings-group">
            <div className="section-heading">
              <h3>Persistence</h3>
              <span>Control what speed is remembered and how it gets reused.</span>
            </div>
            ...
            <label className="toggle-row">
              <div>
                ...
                <p>Reapply the saved or preferred speed when the site resets or recreates media.</p>
              </div>
              <input
                type="checkbox"
                checked={settings.forceSavedSpeedOnLoad}
                onChange={(event) =>
                  void saveSettings({ forceSavedSpeedOnLoad: event.target.checked })
                }
              />
            </label>
          </div>
          ...
          <div className="settings-group">
            ...
            <label className="toggle-row">
              <div>
                ...
                <p>Allow the extension to target audio elements as well as video.</p>
              </div>
              <input
                type="checkbox"
            ...
            <label className="toggle-row">
              <div>
                ...
                <p>Show a near-transparent confirmation after shortcut actions.</p>
              </div>
              <input
                type="checkbox"
          ...
          <div className="settings-group">
            <div className="section-heading">
              ...
              <span>Use one hostname per line. Simple host/domain entries only.</span>
            </div>
            <label className="field">
              <span>Hostnames</span>
              <textarea
                ...
                onBlur={() =>
                  void saveSettings({
                    disabledSites: normalizeRules(disabledSitesValue.split(/\r?\n/)),
                  })
                }
              />
            </label>
          ...
          <ShortcutEditor
            ...
            onChange={(nextShortcuts) => void saveSettings({ shortcuts: nextShortcuts })}
          />
        </section>
      ) : null}
    </div>
