import { getSettings, getLastSavedSpeed, listenForSettingsChanges, setLastSavedSpeed } from "@/core/settings";
...
export default defineContentScript({
  ...
  main() {
    ...
    const resolveSettings = (nextSettings: AppSettings): AppSettings => ({
      ...nextSettings,
      enabled:
        nextSettings.enabled && !isHostnameDisabled(hostname, nextSettings.disabledSites),
    });
    ...
    const handleShortcutAction = async (action: "increase" | "decrease" | "reset" | "preferred") => {
    ...
    const handleRuntimeMessage = async (message: unknown) => {
      ...
      if ((runtimeMessage as ApplyExactSpeedMessage)?.type === "PSC_APPLY_EXACT_SPEED") {
    ...
    const stopListeningToStorage = listenForSettingsChanges(async (nextSettings) => {
